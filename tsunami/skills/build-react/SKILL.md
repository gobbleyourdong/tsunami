...
| `StatGrid` | `{ stats?: {label, value, delta?, ...}[], columns?: number, minWidth?: number, gap?: 0\|1\|2\|3\|4\|6\|8, children?, className? }` — pass `stats` array OR `children` (MetricCards). **`gap` and `columns` are numeric literals, NOT strings** — `<StatGrid gap={4} columns={3}>` passes, `<StatGrid gap="4">` fails TS2322 (`Type 'string' is not assignable to type '0 \| 1 \| 2 \| 3 \| 8 \| 6 \| 4'`). Seen in pomodoro session `tsu_prog_pomodoro_1776358411` — 4 fires in single session (`gap="4"` on StatGrid at lines 104,38 + 124,36 + re-fire lines 141,38 + 161,36). The "use `className='gap-4'` instead" escape is ALSO wrong (scaffold has no Tailwind; see #57 rule). |
| `Flex` | `{ gap?: 0\|1\|2\|3\|4\|6\|8, mt?: number, mb?: number, p?: number, grow?: boolean, children, ...HTMLAttributes }` — same numeric-union discipline as StatGrid. `<Flex gap={2}>` passes, `<Flex gap="2">` / `<Flex gap="sm">` fail TS2322. For `grow`/`mt`/`mb`/`p`, use numeric literals (`mt={4}`, NOT `mt="4"`). Seen in lunchvote `tsu_prog_lunchvote_1776331493` (`gap="2"`) + watchlist `tsu_prog_watchlist_1776357020` (`{ children: Element[]; gap: 2; mt: number }` not assignable to FlexProps — TS2322 on combined numeric-union + extraneous prop). |
...
| `Text` | `{ as?: "span" \| "p" \| "div", ...HTMLAttributes }` — **only** `as` + className. **NO `size`, NO `weight`, NO `bold`, NO `variant`**. Size/weight goes via className (Tailwind scale: `text-sm`/`text-lg`/`text-2xl`, `font-bold`/`font-medium`). Chakra/Mantine/Radix Text convention leaks (`<Text size="xl" bold>`) fail TS2322 |
