...
- **Tailwind v4 is installed in `react-app` ONLY** (`@import "tailwindcss"` in `src/index.css`, `tailwindcss@^4.2.2` + `@tailwindcss/vite@^4.2.2` in `package.json`, `tailwindcss()` plugin in `vite.config.ts`). The 6 non-react-app scaffolds (`dashboard` / `data-viz` / `form-app` / `fullstack` / `landing` / `realtime`) do NOT install Tailwind — their `src/index.css` has no `@import "tailwindcss"` line and their `package.json` has no tailwindcss dep. **Arbitrary Tailwind utilities (`text-3xl`, `grid-cols-3`, `flex-col`, `mx-auto`, `min-h-screen`, `from-slate-950`, `text-green-400/10`, `space-y-4`) do NOT compile to CSS on those scaffolds — they silently become no-op classes at runtime.** The build still PASSES because Tailwind classnames are strings (no TS error); the classes just don't render any styles. This is the undertow `"N/M unresolved Tailwind tokens"` QA:FAIL signature (e.g. 72/97 = 74% on hnresearch `tsu_prog_hnresearch_1776363215` first-shot dashboard-template). On non-react-app scaffolds, use the **scaffold utility classes defined in `src/index.css` only** (`.container`, `.card`, `.flex`, `.flex-col`, `.flex-center`, `.grid`, `.grid-2`, `.grid-3`, `.grid-4`, `.gap-2`, `.gap-4`, `.gap-6`, `.gap-8`, `.p-4`, `.p-6`, `.bg-0`, `.bg-1`, `.bg-2`, `.bg-3`, `.text-muted`, `.text-accent`, `.text-bold`, `.rounded`, `.badge`, `.divider`, `.animate-in`) — or switch scaffolds via `project_init(template="react-app")` if you want full Tailwind. Source: `md5sum scaffolds/*/src/index.css` — only `react-app/src/index.css` contains `@import "tailwindcss"`; `grep tailwindcss scaffolds/{dashboard,data-viz,form-app,fullstack,landing,realtime}/package.json` returns empty.
