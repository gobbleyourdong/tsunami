/**
 * Title scene — pre-boot splash.
 *
 * Mounts ChipMusic (looping title theme) + HUD (press-start prompt).
 * Kicks over to World on "start" input.
 */
...
export class Title {
  ...
  constructor() {
    const title = (config as any).meta?.title ?? 'JRPG Scaffold'
    this.description = `${title} — press Start to begin`
  }
